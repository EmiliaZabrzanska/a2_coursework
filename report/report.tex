...
The Gaussian filtering does make each image appear smoother, with background noise also being reduced. However, this smoothing is uniform across the image which can be seen around the edges and tissue boundaries, which do not look as sharp as in the original, which is not ideal in clinical settings. This is most noticable in coild 1 and 5 where the gaussian image appears blurred. 

The results are very mixed when using bilateral filtering. The background noise is reduced compared to the original images, but the images appear slightly washed out and blurred, especially coil 3, which was the coil with the smallest signal intensity of 0.02-0.12, whereas all other coils ranged from around 0.2-1.4. Coils 1, 5 and 6 do appear sharper, demonstrating that at this range of intensity the filtering works well. 

Wavelet filtering appears to have worked well on all the coils, successfully reducing background noise while still preserving the edge sharpness of all the structures.

Despite their differing levels of success, none of the filters have managed to remove the diagonal streaks in coil 6, which is still apparent in the images. 
...
The phase image is noticibly more smoothed out that in the unfiltered plot in Figure 2.1c, with smooth gradients over the majority of the image, and background noise reduced. The random salt and pepper pattern in still visible around the edges of the plot, but is much better than previously. 

A disadvantage of this filter is that unlike the image based methods, it can only be applied to one coil, which makes direct SNR comaprisons difficult to acheive due to the differing signal intensity ratios of each coil. 
...
Gaussian FIltering produced a slightly improved reduction in noise and was consistent across all 6 coils. It works through convolution with a gaussian kernel, which averages each pixel with its neighbours weighted by their distance. Because of the nature of this filter, it is unable to distinguish between frequencies coming from noise and signal, so results in blurring both. This produced a slight bur across the image, which is particularly notiacable when comparing the edges of the origian with the gaussian filter, as they appear much softer. This is a major disadvantage for MRIimages, as sharp edges are extremely important to be able to identify the anatomical structures present in the scan.
